...
const navigate = useNavigate();
  ...
  let roo = await fetch(`https://intelligent-ailyn-handcricket-e8842259.koyeb.app/getprofilerooms?username=${encodeURIComponent(name)}`);
let rooms = await roo.json();
    if(!rooms.error){
     setTimeout(()=>{
      setLoading(false)
      setUsers(rooms)
...
<button onClick={()=>window.location.reload()} className="w-full bg-slate-900 text-white font-bold py-2 px-4 mt-4 rounded-md transition duration-300">Reload </button>
</div>
</div>
    </div>
...
<HashLink smooth to={`/onlinedual?id=${encodeURIComponent(name)}&&player=${encodeURIComponent(i.player)}&&playerteam=${encodeURIComponent(i.playerteam)}&&computerteam=${encodeURIComponent(i.computerteam)}&&computer=${encodeURIComponent(i.computer)}&&inden=${encodeURIComponent(i.id)}`}>
